...
public class StoneDropoutProbabilityConfig {
    ...
    public ModConfigSpec.ConfigValue<Double> getStoneRate(IntensifyStoneType intensifyStoneType) {
        return stoneRate.get(intensifyStoneType);
    }

    public Optional<Item> dropStone(DropTypeEnum dropTypeEnum, Object key) {
        Double probability = getConfiguredProbability(dropTypeEnum, key) * totalRate.get();
    ...
    public Double getStoneDropOutProbability(
        ...
        Double stoneDropProperties = getConfiguredProbability(dropTypeEnum, key) * totalRate.get();
        return stoneDropProperties * stoneRate;
    }

    private Double getConfiguredProbability(DropTypeEnum dropTypeEnum, Object key) {
        String registryName = resolveLookupKey(dropTypeEnum, key);
        Map<String, ModConfigSpec.ConfigValue<Double>> configuredProbabilities =
                stoneDropProbability.get(dropTypeEnum);
        if (registryName != null) {
            ModConfigSpec.ConfigValue<Double> configuredProbability =
                    configuredProbabilities.get(registryName);
            if (configuredProbability != null) {
                return configuredProbability.get();
            }
        }
        return defaultProbabilities.get(dropTypeEnum).get();
    }

    private static String resolveLookupKey(DropTypeEnum dropTypeEnum, Object key) {
        if (key instanceof ResourceLocation resourceLocation) {
            return resourceLocation.toString();
        }

        if (dropTypeEnum == DropTypeEnum.MINERAL_BLOCK_DESTROYED && key instanceof Block block) {
            return BuiltInRegistries.BLOCK.getKey(block).toString();
        }
        if (dropTypeEnum == DropTypeEnum.FISHING && key instanceof Item item) {
            return BuiltInRegistries.ITEM.getKey(item).toString();
        }
        if (dropTypeEnum == DropTypeEnum.MOB_KILLED && key instanceof EntityType<?> entityType) {
    ...
    private static Item resolveStone(IntensifyStoneType stoneType) {
        return switch (stoneType) {
            case INTENSIFY_STONE -> ItemRegistry.STRENGTHENING_STONE.get();
            case ENENG_STONE -> ItemRegistry.ENENG_STONE.get();
            case STRENGTHENING_STONE -> ItemRegistry.STRENGTHENING_STONE.get();
            case ETERNAL_STONE -> ItemRegistry.ETERNAL_STONE.get();
            case PROTECTION_STONE -> ItemRegistry.PROTECTION_STONE.get();
        };
    }
